...
class SDP_serial():
    ...
    def serial_init(self, port, baudrate, frame_read_timeout=None, frame_write_timeout=None):
        ...
        try:
            ...
            if SDP_DEBUG:
                print('SDP port', port, 'status:', status)

        except Exception as e:
            if SDP_DEBUG:
                print('SDP port', port, 'can\'t be opened, error:', e)
                ...
                if num > 0:
                    ...
                    for d in data:  # append data to buffer, byte by byte
                        if sys.version[0] >= '3':
                            ...
                            # python 2.x
                            self.rx_buff.append(ord(d))
                            ord_data.append(ord(d))

                    if SDP_DEBUG_IO_DATA:
                        print('SDP read: %s:+ ' % ord_data)
...
class SDP():
    ...
    def _receive_parse_thread(self):
        ...
        while not self.__thread_stop_flag:
            self.parse_rx_data()

        self.__thread_stop_flag = False  # reset flag
        if SDP_DEBUG:
...
########################################################################################
    ...
    def __compose_frame(self, payload):
        ...
        if len(frame) > self.__max_frame_size:   # check if frame is inside of SDP size setup
    ...
    ########################################################################################
    def __compose_nack_frame(self, payload):
        """
        Compose frame with payload for response purposes (CRC verification failure at receiver)
        Returns status and array of bytes
        """
        (status, frame) = self.__compose_frame(payload)  # build normal frame
        if status:
    ...
    def __check_data(self, data):
        """
        Checks data elements if they fit in one byte (0 - 255). 
        Data type of each element must be integer or single character. 
        Data is converted to integer (byte).
        Returns True if all elements are OK, False otherwise.
        """
        for i, d in enumerate(data):
            if isinstance(d, str):  # d = character
                if len(d) == 1:  # check if it is not a single character
                    d = ord(d)
                    data[i] = d
                else:  # string received, error
                    self.debug('payload element is string')
                    return False

            elif isinstance(d, int):  # d = integer
                if d > 255:  # check for value
                    self.debug('integer > 255')
                    return False
            else:
                self.debug('invalid element data-type')
                return False
