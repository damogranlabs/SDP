...
#define SDP_RETRANSMIT 1 // number of retries in case of send/receive error
...
typedef enum{
  ...
  SDP_RX_RESPONSE, // waiting for response byte
  SDP_RX_STATUS,  // waiting for status byte
...
typedef struct{
  ...
  // user CAN SET this variables -> in sdp.h or after init() function call
  uint32_t rx_msg_timeout;    // once SOF is received, EOF must arrive in this time, or message is discarded
  uint32_t tx_msg_timeout;    // once transmitt is started, bytes must be streamed out in this time
  uint32_t response_timeout;  // on sdp_send_data() call, receiver must respond in this time 
  ...
  uint8_t _response;  // internal variable that indicates this is a response message
  uint8_t _status;    // internal variable that indicates if message is correctly received (only if received message is response)
