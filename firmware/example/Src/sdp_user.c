...
#include "stm32f0xx_ll_crc.h" // STM32 CRC library

/* User includes ------------------------------------------------------------------*/   
#include "stm32xx_hal_liquid_crystal.h"

#define LED_ON() HAL_GPIO_WritePin(GPIOD, GPIO_PIN_13, GPIO_PIN_SET);
...
void sdp_user_handle_message(SDP_data_t *node, uint8_t *payload, uint8_t size){
  ...
  sdp_send_response(node, payload, size); // send data back to PC
